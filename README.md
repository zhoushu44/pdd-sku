...
```bash
docker pull <用户名>/trae-project:1.5
docker run -d -p 5173:5173 --name trae-project <用户名>/trae-project:1.5
```
...
# 3. 用新镜像启动容器
docker run -d --name trae-project --restart unless-stopped -p 5173:5173 <用户名>/trae-project:1.5
```
